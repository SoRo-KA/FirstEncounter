"""
    Example file show how to launch a describe what you see.
...
async def run(ip: str, api_key: str) -> None:
    """Run the robot and demonstrate a behavior to describe what the robot sees"""
    async with connect(ip=ip, api_key=api_key) as robot:
        await robot_behavior(robot)


async def robot_behavior(robot: Robot) -> None:
    """Demonstrate launching describe vision behavior"""
    mission = robot.describe_what_you_see()
    await mission.completed()
    logger.info("describe what you see finished")
